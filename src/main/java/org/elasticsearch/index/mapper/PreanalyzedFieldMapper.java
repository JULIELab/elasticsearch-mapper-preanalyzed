...
/** A {@link FieldMapper} for full-text fields that have already been analyzed on token-level and are given as
 * TokenStream in JSON format.
 *
 * This class has largely been copied from {@link TextFieldMapper}. Notable exceptions are the extra fieldType
 * "storedFieldType" that only has the function to store the text value of the field (not the preanalyzed token format
 * but the readable string content) and the contents of the parseCreateField() method that processes the JSON token
 * value format.
 *
 */
public class PreanalyzedFieldMapper extends FieldMapper {
    ...
    public static class Builder extends FieldMapper.Builder {
        ...
        private final Parameter<Boolean> index = Parameter.indexParam(m -> ((PreanalyzedFieldMapper) m).index, true);
        private final Parameter<Boolean> store = Parameter.storeParam(m -> ((PreanalyzedFieldMapper) m).store, false);

        final Parameter<SimilarityProvider> similarity = TextParams.similarity(m -> ((PreanalyzedFieldMapper) m).similarity);

        final Parameter<String> indexOptions = TextParams.indexOptions(m -> ((PreanalyzedFieldMapper) m).indexOptions);
        final Parameter<Boolean> norms = TextParams.norms(true, m -> ((PreanalyzedFieldMapper) m).norms);
        final Parameter<String> termVectors = TextParams.termVectors(m -> ((PreanalyzedFieldMapper) m).termVectors);

        final Parameter<Boolean> fieldData = Parameter.boolParam("fielddata", true, m -> ((PreanalyzedFieldMapper) m).fieldData, false);
        final Parameter<FielddataFrequencyFilter> freqFilter = new Parameter<>(
                "fielddata_frequency_filter",
                true,
                () -> DEFAULT_FILTER,
                PreanalyzedFieldMapper::parseFrequencyFilter,
                m -> ((PreanalyzedFieldMapper) m).freqFilter
        );
        final Parameter<Boolean> eagerGlobalOrdinals = Parameter.boolParam(
                "eager_global_ordinals",
                true,
                m -> ((PreanalyzedFieldMapper) m).eagerGlobalOrdinals,
                false
        );

        final Parameter<Boolean> indexPhrases = Parameter.boolParam("index_phrases", false, m -> ((PreanalyzedFieldMapper) m).indexPhrases, false);
        final Parameter<PrefixConfig> indexPrefixes = new Parameter<>(
                "index_prefixes",
                false,
                () -> null,
                PreanalyzedFieldMapper::parsePrefixConfig,
                m -> ((PreanalyzedFieldMapper) m).indexPrefixes
        ).acceptsNull();
        ...
        public Builder(String name, Version indexCreatedVersion, IndexAnalyzers indexAnalyzers) {
            ...
            this.analyzers = new TextParams.Analyzers(
                    indexAnalyzers,
                    m -> ((PreanalyzedFieldMapper) m).indexAnalyzer,
                    m -> (((PreanalyzedFieldMapper) m).positionIncrementGap)
            );
        }
        ...
        private PreanalyzedFieldType buildFieldType(FieldType fieldType, MapperBuilderContext context) {
            ...
            PreanalyzedFieldType ft = new PreanalyzedFieldType(context.buildFullName(name), index.getValue(), store.getValue(), tsi, meta.getValue());
        ...
        private SubFieldInfo buildPrefixInfo(MapperBuilderContext context, FieldType fieldType, PreanalyzedFieldType tft) {
        ...
        private SubFieldInfo buildPhraseInfo(FieldType fieldType, PreanalyzedFieldType parent) {
        ...
        @Override
        public PreanalyzedFieldMapper build(MapperBuilderContext context) {
            FieldType fieldType = TextParams.buildFieldType(index, store, indexOptions, norms, termVectors);
            PreanalyzedFieldType tft = buildFieldType(fieldType, context);
            ...
            // extended this constructor for the fieldTypeIndex type
            return new PreanalyzedFieldMapper(
                    name,
                    fieldTypeText,
                    fieldTypeIndexed,
                    tft,
    ...
    private static final class PrefixFieldType extends StringFieldType {
        ...
        final int minChars;
        final int maxChars;
        final PreanalyzedFieldType parentField;

        PrefixFieldType(PreanalyzedFieldType parentField, int minChars, int maxChars) {
    ...
    public static class PreanalyzedFieldType extends StringFieldType {
        ...
        public PreanalyzedFieldType(String name, boolean indexed, boolean stored, TextSearchInfo tsi, Map<String, String> meta) {
    ...
    protected PreanalyzedFieldMapper(
            String simpleName,
            FieldType storedFieldType,
            FieldType fieldTypeIndexed, PreanalyzedFieldType indexedFieldType,
        ...
        super(simpleName, indexedFieldType, indexAnalyzers, multiFields, copyTo, false, null);
        assert indexedFieldType.getTextSearchInfo().isTokenized();
        assert indexedFieldType.hasDocValues() == false;
        if (fieldTypeIndexed.indexOptions() == IndexOptions.NONE && fieldType().fielddata()) {
            throw new IllegalArgumentException("Cannot enable fielddata on a [preanalyzed] field that is not indexed: [" + name() + "]");
        }
        this.fieldTypeIndexed = fieldTypeIndexed;
        this.storedFieldType = storedFieldType;
    ...
    protected void parseCreateField(DocumentParserContext context) throws IOException {
        ...
        try (XContentParser parser = new JsonXContentParser(null, new NoopDeprecationHandler(), jsonFactory.createParser(value))) {
            try {
            ...
            if (storedFieldType.stored() && null != storedValue.value) {
                Field field;
                if (PreAnalyzedStoredValue.VALUE_TYPE.STRING == storedValue.type) {
                    field = new Field(fieldType().name(), (String) storedValue.value, storedFieldType);
                } else {
                    field = new Field(fieldType().name(), (BytesRef) storedValue.value, storedFieldType);
                }
                context.doc().add(field);
            }
...
//        }
    ...
    private Tuple<PreAnalyzedStoredValue, TokenStream> parsePreAnalyzedFieldContents(XContentParser parser) {
        try {
            ...
            return new Tuple<>(storedValue, ts);
        } catch (IOException e) {
