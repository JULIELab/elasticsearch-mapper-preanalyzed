 ...
 *
 */
public class PreAnalyzedFieldMapper extends AbstractFieldMapper<String>
	...
	public static class Builder extends
			AbstractFieldMapper.OpenBuilder<Builder, PreAnalyzedFieldMapper> {
		...
		public PreAnalyzedFieldMapper build(BuilderContext context) {
			PreAnalyzedFieldMapper fieldMapper = new PreAnalyzedFieldMapper(
					buildNames(context), boost, fieldType, indexAnalyzer,
					searchAnalyzer, provider, similarity, normsLoading, fieldDataSettings);
	...
	public static class TypeParser implements Mapper.TypeParser {
		...
		public Mapper.Builder parse(String name, Map<String, Object> node,
			...
			PreAnalyzedFieldMapper.Builder builder = new PreAnalyzedFieldMapper.Builder(
					name);
	...
	protected PreAnalyzedFieldMapper(Names names, float boost,
			FieldType fieldType, NamedAnalyzer indexAnalyzer,
	...
	protected Field parseCreateField(ParseContext context) throws IOException {
		...
		if (fieldTypeText.stored() && null != storedValue.value) {
			...
			if (PreAnalyzedStoredValue.VALUE_TYPE.STRING == storedValue.type) {
				field = new Field(names.indexName(),
						(String) storedValue.value, fieldTypeText);
			} else {
				field = new Field(names.indexName(),
						(BytesRef) storedValue.value, fieldTypeText);
			}
			context.doc().add(field);
		}
	...
	private Tuple<PreAnalyzedStoredValue, TokenStream> parsePreAnalyzedFieldContents(
			BytesRef preAnalyzedData) {
		try {
			XContentParser parser = XContentHelper.createParser(
					preAnalyzedData.bytes, 0, preAnalyzedData.length);
			...
			while ((currentToken = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
				if (currentToken == XContentParser.Token.FIELD_NAME) {
					...
					if ("v".equals(currentFieldName)) {
						...
						if (!"1".equals(version)) {
							throw new MapperParsingException(
									"Version of pre-analyzed field format is \""
											+ version
			...
			return new Tuple<PreAnalyzedStoredValue, TokenStream>(storedValue,
					new PreAnalyzedTokenStream(preAnalyzedData));
		} catch (IOException e) {
			throw new MapperParsingException(
					"The input document could not be parsed as a preanalyzed field value.",
					e);
		}
	}
